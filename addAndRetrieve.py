...
def checkEntry(name, url, email, usrname):
	db = dbgen()
	cursor = db.cursor()
	query = f"SELECT * FROM passwdDB.entries WHERE name = '{name}' AND url = '{url}' AND email = '{email}' AND usrname = '{usrname}'"
	cursor.excute(query)
...
def addEntry(masterPasswd, saltPhrase, name, url, email, usrname):
	...
	masterKey = computeMasterKey(masterPasswd, saltPhrase)
	...
	cursor.execute("INSERT INTO passwdDB.entries (name, url, email, usrname, password) values (%s, %s, %s, %s, %s, %s)", val)
	db.commit()
...
def getEntry(masterPasswd, saltedPhrase, search, decrypt=False)
	...
	if decrypt and len(results)==1:
		masterKey = compute masterKey(masterPasswd, saltPhrase)
